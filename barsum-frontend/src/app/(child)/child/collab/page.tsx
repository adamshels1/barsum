...
const dict: Dict = {
  ru: {
    ...
    subtitle: "Придумай продолжение сказки своим голосом — лучшие главы попадут в книгу!",
    empty: "Пока нет открытых книг",
    emptyHint: "Загляни позже — эксперты скоро откроют новые истории",
    chapters: "глав",
  ...
  kk: {
    ...
    subtitle: "Ертегінің жалғасын өз дауысыңмен ойлап тап — үздік тараулар кітапқа енеді!",
    empty: "Әзірге ашық кітаптар жоқ",
    emptyHint: "Кейінірек кіріп көр — сарапшылар жаңа әңгімелер ашады",
    chapters: "тарау",
...
export default function ChildCollabListPage() {
  const t = useT(dict);
      ...
      {isLoading ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {books.map((b) => (
            <button
              key={b.id}
              onClick={() => router.push(`/child/collab/${b.id}`)}
              className="glass"
            ...
            >
              <div style={{ width: 52, height: 52, borderRadius: 14, background: "rgba(255,255,255,0.12)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26, flexShrink: 0 }}>
                {b.coverImage ? (
                  ...
                  <img src={b.coverImage} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: 14 }} />
                ) : "✨"}
              </div>
