...
const dict: Dict = {
  ru: {
    childLabel: "Ребёнок",
    childSub: "Читай книги, зарабатывай монеты",
    parentLabel: "Родитель",
    parentSub: "Мотивируй своего ребёнка",
    expertLabel: "Эксперт",
    expertSub: "Создавай задания для детей",
    tagline: "Читай · Зарабатывай · Мечтай",
    footer: "Образовательная платформа · Казахстан",
  },
  kk: {
    childLabel: "Бала",
    childSub: "Кітап оқы, монета жинай",
    parentLabel: "Ата-ана",
    parentSub: "Балаңды ынталандыр",
    expertLabel: "Сарапшы",
    expertSub: "Балаларға тапсырмалар жаса",
    tagline: "Оқы · Тап · Арманда",
    footer: "Білім беру платформасы · Қазақстан",
  },
};
  ...
  return (
    ...
    >
      ...
      <div style={{ width: "100%", maxWidth: 380, display: "flex", flexDirection: "column", gap: 12, position: "relative", zIndex: 1 }}>
        {ROLES.map(({ key, labelKey, subKey, Icon, gradient, href }) => (
          <button
            key={key}
          ...
          >
            ...
            <div style={{ flex: 1 }}>
              <p style={{ color: "#ffffff", fontWeight: 800, fontSize: 17, margin: 0 }}>{t(labelKey)}</p>
              <p style={{ color: "rgba(255,255,255,0.48)", fontSize: 13, fontWeight: 500, margin: "3px 0 0" }}>{t(subKey)}</p>
            </div>
