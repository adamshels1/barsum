...
export function RewardRequestCard({ request, highlight }: { request: RewardRequest; highlight?: boolean }) {
  ...
  const deliverMutation = useMutation({
    ...
    onSuccess: () => { invalidate(); toast.success("Награда выдана! 🎉"); },
    onError: (err: any) => toast.error(err?.response?.data?.message || "Ошибка"),
  });

  const rejectMutation = useMutation({
    mutationFn: () => rewardsApi.reject(request.id),
    onSuccess: () => { invalidate(); toast.success("Запрос отклонён, монеты возвращены"); },
    onError: (err: any) => toast.error(err?.response?.data?.message || "Ошибка"),
  });
  ...
  return (
    <div style={{ ...GLASS, padding: "14px 16px", background: highlight ? "rgba(255,255,255,0.22)" : GLASS.background }}>
      <div style={{ display: "flex", alignItems: "flex-start", gap: 12, marginBottom: 12 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <p style={{ margin: "2px 0 0", fontSize: 13, color: "rgba(255,255,255,0.7)" }}>
            хочет: <span style={{ fontWeight: 700, color: "#ffffff" }}>{rewardName}</span>
          </p>
        ...
        <span style={{ fontSize: 11, padding: "4px 10px", borderRadius: 9999, fontWeight: 800, background: "rgba(255,200,0,0.3)", color: "#ffffff", flexShrink: 0 }}>ожидает</span>
      </div>
      <div style={{ display: "flex", gap: 8 }}>
        ...
        >
          {deliverMutation.isPending ? "..." : "✅ Подтвердить"}
        </button>
        <button
