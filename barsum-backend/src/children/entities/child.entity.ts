...
export class Child {
  ...
  // Когда ребёнка один раз спросили «на что копишь?» после засчитанного чтения.
  // Отдельно от onboardedAt: спрашиваем тех, кто уже читает, но мечты не завёл,
  // и повторно не пристаём независимо от ответа.
  @Column({ nullable: true, type: 'timestamp' })
  dreamPromptedAt: Date | null;
